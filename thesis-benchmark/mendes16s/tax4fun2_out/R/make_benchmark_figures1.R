...
picru_tsv <- file.path(PROJ_DIR, "picrust2", "picrust2_out",
                       "KO_metagenome_out", "pred_metagenome_unstrat.tsv.gz")

# this is the file your professor sent (run_accession, sample_title)
map_tsv   <- file.path(PROJ_DIR, "analysis", "correct_map_samples.tsv")

stopifnot(file.exists(tax_tsv),
          file.exists(picru_tsv),
          file.exists(SHOTGUN_TSV),
          file.exists(map_tsv))
...
# ---------- sample map + groups (USING ONLY run_accession + sample_title) ----
smap <- fread(map_tsv)  # columns: run_accession, sample_title

# rename to what the rest of the code expects
setnames(smap,
         old = c("run_accession", "sample_title"),
         new = c("err", "shotgun_id"))

# derive Bulk / RAG / RTP from sample_title
smap[, group := fifelse(grepl("^Bulk", shotgun_id, ignore.case = TRUE), "Bulk",
                        fifelse(grepl("^RAG",  shotgun_id, ignore.case = TRUE), "RAG",
                                fifelse(grepl("^RTP",  shotgun_id, ignore.case = TRUE), "RTP",
                                        "OTHER")))]
...
if (length(missing))
  stop("sample_map is missing columns: ", paste(missing, collapse=", "))
...
# PICRUSt2 vs Shotgun
pairs_pic <- smap[err %in% colnames(picM) & shotgun_id %in% colnames(wgsM),
                  .(err, shotgun_id, group)]

# PICRUSt2 vs Tax4Fun2 (ERR on both sides)
...
fwrite(col_tax_wgs,
       file.path(S16_DIR, "tax4fun2_out", "colwise_tax4fun2_vs_shotgun.csv"))

p_tax_spear <- ggplot(col_tax_wgs, aes(spearman)) +
  geom_histogram(bins = 18) +
  labs(title = "Tax4Fun2 vs Shotgun — Spearman (per sample)",
       x = "Spearman", y = "Count")

p_tax_jacc  <- ggplot(col_tax_wgs, aes(jaccard)) +
  geom_histogram(bins = 18) +
  labs(title = "Tax4Fun2 vs Shotgun — Jaccard (per sample)",
       x = "Jaccard", y = "Count")
...
# ---------- 2) Column-wise PICRUSt2 vs Shotgun ----------
message("Computing PICRUSt2 vs Shotgun metrics …")
col_pic_wgs <- rbindlist(lapply(seq_len(nrow(pairs_pic)), function(i){
  e <- pairs_pic$err[i]
  s <- pairs_pic$shotgun_id[i]
  g <- pairs_pic$group[i]
  x <- wgsM[, s, drop = TRUE]
  y <- picM[, e, drop = TRUE]
  data.table(sample   = e,
...
fwrite(col_pic_wgs,
       file.path(S16_DIR, "tax4fun2_out", "colwise_picrust2_vs_shotgun.csv"))

p_pic_spear <- ggplot(col_pic_wgs, aes(spearman)) +
  geom_histogram(bins = 18) +
  labs(title = "PICRUSt2 vs Shotgun — Spearman (per sample)",
       x = "Spearman", y = "Count")

p_pic_jacc  <- ggplot(col_pic_wgs, aes(jaccard)) +
  geom_histogram(bins = 18) +
  labs(title = "PICRUSt2 vs Shotgun — Jaccard (per sample)",
       x = "Jaccard", y = "Count")

ggsave(file.path(S16_DIR,"tax4fun2_out","colwise_picrust2_vs_shotgun_spearman_hist.png"),
       p_pic_spear, width = 6.5, height = 4.2, dpi = 220)
ggsave(file.path(S16_DIR,"tax4fun2_out","colwise_picrust2_vs_shotgun_jaccard_hist.png"),
       p_pic_jacc, width = 6.5, height = 4.2, dpi = 220)

# ---------- 3) Column-wise PICRUSt2 vs Tax4Fun2 ----------
...
fwrite(col_pt,
       file.path(S16_DIR, "tax4fun2_out", "colwise_picrust2_vs_tax4fun2.csv"))

p_pt_spear <- ggplot(col_pt, aes(spearman)) +
  geom_histogram(bins = 18) +
  labs(title = "PICRUSt2 vs Tax4Fun2 — Spearman (per sample)",
       x = "Spearman", y = "Count")

p_pt_jacc  <- ggplot(col_pt, aes(jaccard)) +
  geom_histogram(bins = 18) +
  labs(title = "PICRUSt2 vs Tax4Fun2 — Jaccard (per sample)",
       x = "Jaccard", y = "Count")
...
# ---------- 4) Group-wise violin plots ----------
v_tax_spear <- ggplot(col_tax_wgs, aes(group, spearman)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = .12, outlier.size = 0.7) +
  labs(title = "Tax4Fun2 vs Shotgun — Spearman by group",
       x = NULL, y = "Spearman")

v_tax_jacc  <- ggplot(col_tax_wgs, aes(group, jaccard)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = .12, outlier.size = 0.7) +
  labs(title = "Tax4Fun2 vs Shotgun — Jaccard by group",
       x = NULL, y = "Jaccard")
...
# ---------- 5) Final 6-panel figure ----------
final_panel <- (p_tax_spear | p_tax_jacc) /
  (p_pic_spear | p_pic_jacc) /
  (p_pt_spear  | p_pt_jacc)

ggsave(file.path(S16_DIR,"tax4fun2_out","final_benchmark_panelF.png"),
       final_panel, width = 11.5, height = 12, dpi = 220)

message("Done.
- CSVs in mendes16s/tax4fun2_out/
- Individual histograms + violin plots saved.
- Final 6-panel figure: mendes16s/tax4fun2_out/final_benchmark_panelF.png")
